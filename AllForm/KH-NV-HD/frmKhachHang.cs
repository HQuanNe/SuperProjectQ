            ...
            try
            {
                ...
                if (flag == true)
                {
                    ...
                    else
                    {
                        ...
                        {
                            string sqlAdd = "INSERT INTO KhachHang(MaKH, TenKH, DiaChi, SoDienThoai, VIP, DiemTichLuy, Discount) values (@MaKH, @TenKH, @DiaChi, @SoDienThoai, @VIP, @DiemTichLuy, @Discount)";
                            ...
                            cmd.Parameters.AddWithValue("@VIP", txtVIP.Text.Trim());
                            cmd.Parameters.AddWithValue("@DiemTichLuy", txtDTL.Text.Trim());
                            cmd.Parameters.AddWithValue("@Discount", txtDiscount.Text.Trim().Replace("%", ""));
                            cmd.ExecuteNonQuery();
                            Load_DB();
                            FocusDataByID(txtMaKH.Text.Trim());
                ...
                else if (flag == false)
                {
                    ...
                    else
                    {
                        ...
                        {
                            string sqlEdit = "" +
                                "UPDATE  KhachHang SET MaKH = (@MaKH), TenKH = (@TenKH), DiaChi = (@DiaChi), SoDienThoai = (@SoDienThoai), VIP = (@VIP), DiemTichLuy = (@DiemTichLuy), Discount = (@Discount)  WHERE MaKH = (@MaKH)";
